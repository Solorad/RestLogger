...
public class ApplicationService {
    ...
    public ListenableFuture<ResponseEntity<?>> registerEndpoint(String displayName) {
        logger.debug("register endpoint with displayName '{}'", displayName);
        if (displayName == null) {
    ...
    public ListenableFuture<ResponseEntity<?>> writeLog(String accessToken, LogRequest logRequest) {
        ...
        if (lastAppAuth.size() != 1
            || (authentication = lastAppAuth.get(0)).getAuthenticationTime().until(LocalDateTime.now(), ChronoUnit.MINUTES)
               > authenticationService.getSessionLifeTimeMin()
                || !accessToken.equals(authentication.getAccessToken())) {
...
//        Authentication authentication = lastAppAuth.get(0);
        Log log = new Log(authentication.getApplication(), logRequest.getLogger(), logRequest.getLevel(), logRequest.getMessage());
